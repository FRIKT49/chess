$('#fileInput').on('change', function() {
    ...
    if (file && file.type.startsWith('image/')) {
        ...
        const cutSize = 200;
            ...
            // Позиционируем квадрат за мышкой
        ...
        $avatar.on('click', function(e) {
            ...
            // Показываем результат (можно заменить window.open на другую обработку)
            const dataURL = canvas.toDataURL('image/png');

            $(img).attr('src', dataURL);
            $cut.hide();
            imgUrl = dataURL;
            // Здесь можно реализовать отправку dataURL на сервер
        });
        ...
        // Кнопка "Сохранить" (пример: просто закрыть окно)
        $('#saveAvatar').on('click', function() {
            // Здесь можно реализовать отправку dataURL на сервер
            $('#avatar img').attr('src',imgUrl ); // Обновляем изображение
            $('#avatarCut').remove();
        });
