    ...
    for (let i = 0; i < count; i++) {
        ...
        figureClone.on('mousedown', function (pos) {
            ...
            $('body').on('mousemove', function (move) {
                const deck = $('#deck');
                const deckWidth = deck.width();
                const offset = deck.offset();
                ...
                // Инвертируем координаты для повёрнутого блока (180 градусов)
                const rotatedX = deckWidth - mouseX;
                const rotatedY = deckHeight - mouseY;
                $('.figure').css({
                    'pointer-events': 'none',
                    'z-index': 9
                })
                ...
                figureClone.css({
                    'position': 'fixed',
                    'left': offset.left + rotatedX - cellWidth / 2 + 'px',
                    'top': offset.top + rotatedY - cellHeight / 2 + 'px'
